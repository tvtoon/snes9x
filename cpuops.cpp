...
{
	uint8	val = S9xGetByte(Registers.PC.xPBPC);
	if (a & READ)
		OpenBus = val;
	Registers.PC.W.xPC++;
...
{
	uint8	val = CPU.PCBase[Registers.PC.W.xPC];
	if (a & READ)
		OpenBus = val;
...
{
	uint16	val = S9xGetWord(Registers.PC.xPBPC, WRAP_BANK);
	if (a & READ)
		OpenBus = (uint8) (val >> 8);
...
{
	uint16	val = READ_WORD(CPU.PCBase + Registers.PC.W.xPC);
	if (a & READ)
		OpenBus = (uint8) (val >> 8);
...
static inline uint32 RelativeSlow (AccessMode a)						// branch $xx
{
	...
	return ((int16) Registers.PC.W.xPC + offset) & 0xffff;
}
...
static inline uint32 RelativeLongSlow (AccessMode a)					// BRL $xxxx
{
	...
	return ((int32) Registers.PC.W.xPC + offset) & 0xffff;
}
...
{
	uint32	addr = READ_3WORD(CPU.PCBase + Registers.PC.W.xPC);
...
{
	uint32	addr = S9xGetWord(DirectSlow(READ), (!CheckEmulation() || Registers.D.B.l) ? WRAP_BANK : WRAP_PAGE);
	if (a & READ)
...
{
	uint32	addr = S9xGetWord(DirectSlow(READ), Registers.D.B.l ? WRAP_BANK : WRAP_PAGE);
	if (a & READ)
		OpenBus = (uint8) (addr >> 8);
	addr |= ICPU.ShiftedDB;
...
{
	uint32	addr = S9xGetWord(DirectIndexedXSlow(READ), (!CheckEmulation() || Registers.D.B.l) ? WRAP_BANK : WRAP_PAGE);
	if (a & READ)
		OpenBus = (uint8) (addr >> 8);
...
{
	uint32	addr = S9xGetWord(DirectIndexedXE1(READ), Registers.D.B.l ? WRAP_BANK : WRAP_PAGE);
	if (a & READ)
		OpenBus = (uint8) (addr >> 8);
...
static inline void ADC (uint8 Work8)
{
	if (CheckDecimal())
	{
		...
		result = (Registers.A.B.l & 0x0F) + (Work8 & 0x0F) + carry;
		if ( result > 0x09 )
			result += 0x06;
		carry = (result > 0x0F);
	...
	{
		uint16	Ans16 = Registers.A.B.l + Work8 + CheckCarry();

		ICPU._Carry = Ans16 >= 0x100;

		if (~(Registers.A.B.l ^ Work8) & (Work8 ^ (uint8) Ans16) & 0x80)
			SetOverflow();
		else
			ClearOverflow();

		Registers.A.B.l = (uint8) Ans16;
		SetZN(Registers.A.B.l);
	}
}
...
{
	int16	Int16 = (int16) Registers.A.B.l - (int16) val;
	ICPU._Carry = Int16 >= 0;
	SetZN((uint8) Int16);
}
...
{
	int16	Int16 = (int16) Registers.X.B.l - (int16) val;
	ICPU._Carry = Int16 >= 0;
	SetZN((uint8) Int16);
}
...
{
	int16	Int16 = (int16) Registers.Y.B.l - (int16) val;
	ICPU._Carry = Int16 >= 0;
	SetZN((uint8) Int16);
}
...
static inline void SBC (uint8 Work8)
{
	...
	{
		int16	Int16 = (int16) Registers.A.B.l - (int16) Work8 + (int16) CheckCarry() - 1;

		ICPU._Carry = Int16 >= 0;

		if ((Registers.A.B.l ^ Work8) & (Registers.A.B.l ^ (uint8) Int16) & 0x80)
			SetOverflow();
		else
			ClearOverflow();

		Registers.A.B.l = (uint8) Int16;
		SetZN(Registers.A.B.l);
	}
}
...
static void Op29Slow (void)
{
	if (CheckMemory())
	{
		Registers.A.B.l &= Immediate8Slow(READ);
		SetZN(Registers.A.B.l);
	}
	else
	{
...
{
	AddCycles(ONE_CYCLE);
	ICPU._Carry = (Registers.A.B.h & 0x80) != 0;
	Registers.A.W <<= 1;
	SetZN(Registers.A.W);
}
...
{
	if (CheckMemory())
		ICPU._Zero = Registers.A.B.l & Immediate8Slow(READ);
	else
...
static void OpC9Slow (void)
{
	if (CheckMemory())
	{
		int16	Int16 = (int16) Registers.A.B.l - (int16) Immediate8Slow(READ);
		ICPU._Carry = Int16 >= 0;
		SetZN((uint8) Int16);
	}
	else
...
static void OpE0Slow (void)
{
	if (CheckIndex())
	{
		int16	Int16 = (int16) Registers.X.B.l - (int16) Immediate8Slow(READ);
		ICPU._Carry = Int16 >= 0;
		SetZN((uint8) Int16);
	}
	else
...
static void OpC0Slow (void)
{
	if (CheckIndex())
	{
		int16	Int16 = (int16) Registers.Y.B.l - (int16) Immediate8Slow(READ);
		ICPU._Carry = Int16 >= 0;
		SetZN((uint8) Int16);
	}
	else
...
static void OpA2Slow (void)
{
	if (CheckIndex())
	{
		Registers.X.B.l = Immediate8Slow(READ);
		SetZN(Registers.X.B.l);
	}
	else
	{
...
static void OpA0Slow (void)
{
	if (CheckIndex())
	{
		Registers.Y.B.l = Immediate8Slow(READ);
		SetZN(Registers.Y.B.l);
	}
	else
	{
...
{
	AddCycles(ONE_CYCLE);
	uint16	w = (((uint16) Registers.A.B.l) << 1) | CheckCarry();
	ICPU._Carry = w >= 0x100;
...
{
	AddCycles(ONE_CYCLE);
	uint16	w = ((uint16) Registers.A.B.l) | (((uint16) CheckCarry()) << 8);
	ICPU._Carry = w & 1;
	w >>= 1;
	Registers.A.B.l = (uint8) w;
	SetZN(Registers.A.B.l);
...
{
	AddCycles(ONE_CYCLE);
	Registers.X.B.l = Registers.A.B.l;
	SetZN(Registers.X.B.l);
}
...
{
	AddCycles(ONE_CYCLE);
	Registers.Y.B.l = Registers.A.B.l;
	SetZN(Registers.Y.B.l);
}
...
{
	AddCycles(ONE_CYCLE);
	Registers.X.B.l = Registers.S.B.l;
	SetZN(Registers.X.B.l);
}
...
{
	AddCycles(ONE_CYCLE);
	Registers.A.B.l = Registers.X.B.l;
	SetZN(Registers.A.B.l);
}
...
{
	AddCycles(ONE_CYCLE);
	Registers.Y.B.l = Registers.X.B.l;
	SetZN(Registers.Y.B.l);
}
...
{
	AddCycles(ONE_CYCLE);
	Registers.A.B.l = Registers.Y.B.l;
	SetZN(Registers.A.B.l);
}
...
{
	AddCycles(ONE_CYCLE);
	Registers.X.B.l = Registers.Y.B.l;
	SetZN(Registers.X.B.l);
}
...
{
	uint8	Work8 = Registers.A.B.l;
	Registers.A.B.l = Registers.A.B.h;
	Registers.A.B.h = Work8;
	SetZN(Registers.A.B.l);
	AddCycles(TWO_CYCLES);
}
...
{
#ifdef DEBUGGER
	uint8	byte = (uint8) S9xGetWord(Registers.PC.xPBPC);
#else
	...
	switch (byte)
	{
		...
		case 0xe2: // "SEP" = Trace on
			if (!(CPU.Flags & TRACE_FLAG))
			{
				...
				snprintf(buf, 25, "WDM trace on at $%02X:%04X", Registers.PC.B.xPB, Registers.PC.W.xPC);
		...
		case 0xc2: // "REP" = Trace off
			if (CPU.Flags & TRACE_FLAG)
			{
				char	buf[26];
				CPU.Flags &= ~TRACE_FLAG;
				snprintf(buf, 26, "WDM trace off at $%02X:%04X", Registers.PC.B.xPB, Registers.PC.W.xPC);
				S9xMessage(S9X_DEBUG, S9X_DEBUG_OUTPUT, buf);
				if (trace != NULL)
					fclose(trace);
				trace = NULL;
		...
		case 0x42: // "WDM" = Snapshot
			...
			snprintf(filename, PATH_MAX, "%s%s%s-%06X.wdm", S9xGetDirectory(SNAPSHOT_DIR), SLASH_STR, def, Registers.PC.xPBPC & 0xffffff);
			sprintf(def, "WDM Snapshot at $%02X:%04X: %s", Registers.PC.B.xPB, Registers.PC.W.xPC, filename);

...
{
 return ((((((C1) & RGB_REMOVE_LOW_BITS_MASK) + ((C2) & RGB_REMOVE_LOW_BITS_MASK)) >> 1) + ((C1) & (C2) & RGB_LOW_BITS_MASK)) | ALPHA_BITS_MASK);
}
...
static void DO_BG( const unsigned char n, const unsigned char hires, const unsigned char offset, const unsigned char Zh, const unsigned char Zl, const unsigned char voffoff )
{
 ...
 if (offset)
{
  ...
  if (PPU.BGMosaic[n] && (hires || PPU.Mosaic > 1)) DrawBackgroundOffsetMosaic(n, Zh, Zl, voffoff);
  else DrawBackgroundOffset(n, Zh, Zl, voffoff);
}
 else
{
  if (PPU.BGMosaic[n] && (hires || PPU.Mosaic > 1)) DrawBackgroundMosaic(n, Zh, Zl);
  else DrawBackground(n, Zh, Zl);
}

}

static void RenderScreen (bool8 sub)
{
/*
 HiRes setting, how many loops, offset setting, pal values (mode 0 exclusive), voff setting, and zh and zl default sums.
 Zh and Zl sum when less than 2, more than...
 Zh got one extra, for when BG3Priority is set and BGMode is 1.
*/
 const unsigned char hiresa[7] = { 0, 0, 0, 0, 0, 1, 1 }, loopa[7] = { 4, 3, 2, 2, 2, 2, 1 }, offseta[7] = { 0, 0, 1, 0, 1, 0, 1 }, pala[4] = { 0, 0, 0, 0 }, pala0[4] = { 0, 32, 64, 96 }, voffa[7] = { 0, 0, 8, 0, 0, 0, 8 }, zhsuma1[4] = { 15, 14, 7, 6 }, zhsuma1p[4] = { 15, 14, 17, 0 }, zhsuma2[4] = { 15, 11, 0, 0 }, zlsuma1[4] = { 11, 10, 3, 2 }, zlsuma2[4] = { 7, 3, 0, 0 };

 const unsigned char deptha[7][4] =
{
 { 2, 2, 2, 2 },
 { 4, 4, 2, 0 },
 { 4, 4, 0, 0 },
 ...
 const unsigned char *palp = pala, *zhsum = zhsuma2, *zlsum = zlsuma2;
 unsigned char ui = 0;
 ...
 else
{
 ...
 if ( PPU.BGMode < 2 )
{
  if ( PPU.BGMode == 0 ) palp = pala0;

  if ( ( PPU.BGMode == 1 ) && ( PPU.BG3Priority ) ) zhsum = zhsuma1p;
  else zhsum = zhsuma1;

  zlsum = zlsuma1;
}

  for ( ; ui < loopa[PPU.BGMode]; ui++ )
{
   if (BGActive & (1 << ui))
{
    S9xSelectTileConverter( deptha[PPU.BGMode][ui], hiresa[PPU.BGMode], sub, PPU.BGMosaic[ui]);
    BG.StartPalette = palp[ui];
    BG.EnableMath = !sub && (Memory.FillRAM[0x2131] & (1 << ui));
    DO_BG( ui, hiresa[PPU.BGMode], offseta[PPU.BGMode], D + zhsum[ui], D + zlsum[ui], voffa[PPU.BGMode] );
}

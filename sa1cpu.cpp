...
void S9xSA1MainLoop (void)
{
	...
	for (; SA1.Cycles < cycles && !(Memory.FillRAM[0x2200] & 0x60);)
	{
		...
		if (SA1.PCBase)
		{
			SA1OpenBus = Op = SA1.PCBase[Registers.PC.W.xPC];
			Opcodes = SA1.S9xOpcodes;
			SA1.Cycles += SA1.MemSpeed;
		}
		else
		{
			Op = S9xSA1GetByte(Registers.PC.xPBPC);
			Opcodes = S9xOpcodesSlow;
		}

		if ((SA1Registers.PC.W.xPC & MEMMAP_MASK) + SA1.S9xOpLengths[Op] >= MEMMAP_BLOCK_SIZE)
		{
			uint32	oldPC = SA1Registers.PC.xPBPC;
			S9xSA1SetPCBase(SA1Registers.PC.xPBPC);
			SA1Registers.PC.xPBPC = oldPC;

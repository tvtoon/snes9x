...
void S9xSA1Init (void)
{
	...
	SA1Registers.PC.xPBPC = 0;
	SA1Registers.PC.B.xPB = 0;
	SA1Registers.PC.W.xPC = 0;
	SA1Registers.D.W = 0;
	SA1Registers.DB = 0;
	SA1Registers.S.B.h = 1;
	SA1Registers.S.B.l = 0xFF;
	SA1Registers.X.B.h = 0;
	SA1Registers.Y.B.h = 0;
	SA1Registers.P.W = 0;
	...
	S9xSA1SetPCBase(SA1Registers.PC.xPBPC);
...
{
	SA1.ShiftedPB = (uint32) SA1Registers.PC.B.xPB << 16;
	...
	switch (address)
	{
		case 0x2200: // SA-1 control
			...
			if (!(byte & 0x80) && (Memory.FillRAM[0x2200] & 0x20))
			{
				...
				SA1Registers.PC.xPBPC = 0;
				SA1Registers.PC.B.xPB = 0;
				SA1Registers.PC.W.xPC = Memory.FillRAM[0x2203] | (Memory.FillRAM[0x2204] << 8);
				S9xSA1SetPCBase(SA1Registers.PC.xPBPC);
			}
...
{
	SA1Registers.PC.xPBPC = address & 0xffffff;
...
void S9xSA1UnpackStatus (void)
{
	SA1._Zero = (SA1Registers.P.B.l & Zero) == 0;
	SA1._Negative = (SA1Registers.P.B.l & Negative);
	SA1._Carry = (SA1Registers.P.B.l & Carry);
	SA1._Overflow = (SA1Registers.P.B.l & Overflow) >> 6;
}

void S9xSA1PackStatus (void)
{
	SA1Registers.P.B.l &= ~(Zero | Negative | Carry | Overflow);
	SA1Registers.P.B.l |= SA1._Carry | ((SA1._Zero == 0) << 1) | (SA1._Negative & 0x80) | (SA1._Overflow << 6);
}

void S9xSA1FixCycles (void)
{

...
static const int32 *hblankeva[7] = { 0, &Timings.HDMAStart, &Timings.H_Max, &Timings.HDMAInit, &Timings.RenderPos, &Timings.WRAMRefreshPos, &Timings.HBlankStart };

static void S9xReschedule (void)
{
 CPU.NextEvent = hblankeva[CPU.WhichEvent][0];

 if ( CPU.WhichEvent == 6 ) CPU.WhichEvent = HC_HBLANK_START_EVENT;
 else CPU.WhichEvent++;
}
...
void S9xMainLoop (void)
{
	...
	for (;;)
	{
		...
		if ((Registers.PC.W.xPC & MEMMAP_MASK) + ICPU.S9xOpLengths[Op] >= MEMMAP_BLOCK_SIZE)
		{
			...
			CPU.PCBase = S9xGetBasePointer(ICPU.ShiftedPB + ((uint16) (Registers.PC.W.xPC + 4)));
/*			if (oldPCBase != CPU.PCBase || (Registers.PCw & ~MEMMAP_MASK) == (0xffff & ~MEMMAP_MASK))*/
			if ( ( oldPCBase != CPU.PCBase ) || ( Registers.PC.W.xPC == 0xF000 ) ) Opcodes = S9xOpcodesSlow;
		}

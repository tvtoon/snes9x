...
static void addCyclesInMemoryAccess( const int32 speed )
{
...
static int32 memory_speed ( const uint32 address )
{
	if (address & 0x408000)
	{
		if (address & 0x800000)
...
uint16 S9xGetWord ( const uint32 Address, const enum s9xwrap_t w )
{
	uint16	word;
	...
	if (GetAddress >= (uint8 *) CMemory::MAP_LAST)
	{
		...
		addCyclesInMemoryAccess( speed << 1 );
		return (word);
	}
...
void S9xSetWord ( const uint16 Word, const uint32 Address, const enum s9xwrap_t w, const enum s9xwriteorder_t o )
{
	uint32	mask = MEMMAP_MASK & (w == WRAP_PAGE ? 0xff : (w == WRAP_BANK ? 0xffff : 0xffffff));
	if ((Address & mask) == mask)
	{
